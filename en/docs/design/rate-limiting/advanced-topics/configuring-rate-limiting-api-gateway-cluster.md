...
!!! note
    This capability is available from U2 level 368 onwards. (wso2am-4.0.0.368)
...
Open the `<API-M_HOME>/repository/conf/deployment.toml` file of the Gateway node and add the following configurations to the `deployment.toml` file.
...
| database_id                | The Redis logical database index (`SELECT n`) used for throttling data. Use a dedicated index to isolate throttle keys from other Redis tenants on the same server.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Optional               | 0                 | 0                 |
...
| connection_timeout         | The timeout value (in ms) for establishing a connection to the Redis server. <br/>If this is not configured, or is set to 0, the Jedis default value will be applied.                  | Optional               | 2000              | 2000     |
| socket_timeout              | The timeout value (in ms) for socket operations when communicating with the Redis server. <br/>If this is not configured, or is set to 0, the Jedis default value will be applied.     | Optional               | 2000              | 2000     |
| ssl                         | Whether to connect to the Redis server over TLS. Enable this for managed Redis offerings that mandate in-transit encryption.                                                                                                          | Optional               | false             | false             |
| pool_options.MaxTotal                      | Maximum number of total Redis connections (active + idle) that can be allocated by the connection pool at a time.                                                                     | Optional           | 50           | 8             |
| pool_options.MaxIdle                       | Maximum number of idle Redis connections that can remain in the pool without being closed.                                                                                            | Optional           | 20           | 8             |
| pool_options.MinIdle                       | Minimum number of idle Redis connections that should be maintained in the pool.                                                                                                       | Optional           | 5            | 0             |
| pool_options.max_wait_millis               | The maximum amount of time, in milliseconds, to wait for a connection to become available from the pool before an exception is thrown. | Mandatory           | 3000         | -1 (Redis default) |
|yu | X |  | P |
| pool_options.BlockWhenExhausted            | Determines whether the client should wait (block) when the connection pool is exhausted. If set to `false`, a connection request fails immediately when no connections are available. | Optional           | true         | true          |
| pool_options.TestOnBorrow                  | Indicates whether a connection should be validated before being borrowed from the pool. Helps avoid returning broken connections.                                                     | Optional           | true         | false         |
| pool_options.TestOnReturn                  | Indicates whether a connection should be validated when returned to the pool.                                                                                                         | Optional           | false        | false         |
| pool_options.TestWhileIdle                 | Specifies whether idle connections should be validated periodically during eviction runs.                                                                                             | Optional           | true         | true          |
| pool_options.MinEvictableIdleTimeMillis    | Minimum amount of time (in milliseconds) a connection may sit idle in the pool before it becomes eligible for eviction.                                                               | Optional           | 120000       | 60000         |
| pool_options.TimeBetweenEvictionRunsMillis | Time interval (in milliseconds) between idle connection eviction runs. If set to a negative value, eviction will not run.                                                             | Optional           | 60000        | 30000         |
| pool_options.NumTestsPerEvictionRun        | Number of connections to examine during each eviction run. A negative value means all idle connections will be tested.                                                                | Optional           | 3            | -1            |

!!! warning
    `pool_options.max_wait_millis` defaults to `-1`, which is the underlying Redis/Jedis client library's own default (block indefinitely, no timeout) — it is not a value tuned for this product, and `-1` is not recommended in any deployment. This connection pool is shared by every feature that uses `[apim.redis_config]` (Hybrid Mode throttling, OAuth 2.0 Redis-backed token caching, etc.), so the risk isn't specific to Hybrid Mode. Combined with `block_when_exhausted = true` (also the default) and a small connection pool (`max_total = 8` by default), a slow or unresponsive Redis server can cause gateway request-processing threads to block forever waiting for a pooled connection, eventually exhausting the gateway's worker thread pool and making it unresponsive to all traffic. Always set `max_wait_millis` explicitly to a bounded value appropriate for your deployment before going to production.
